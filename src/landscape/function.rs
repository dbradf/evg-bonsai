...
fn cmd_list_to_function_def(cmd_list: &[BonsaiCommand], bonsai_cmds: &HashMap<String, FunctionDefinition>) -> Vec<EvgCommand> {
    let mut evg_cmd_list = vec![];
    cmd_list.iter().for_each(|bc| {
        match bc {
...
pub fn translate_fn_def(fn_def: &BonsaiFunctionDef, bonsai_fns: &HashMap<String, FunctionDefinition>) -> FunctionDefinition {
    match fn_def {
...
pub fn translate_functions(
    ...
    for (k, v) in fn_map {
        new_map.insert(k.clone(), translate_fn_def(v, bonsai_fns));
    }
    new_map
}
